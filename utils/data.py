...
class LabeledDocuments(Data):
    ...
    def load_datasets(self):
        self.X_train, self.Y_train = self.read_data(
            f"{self.file_path}/train.csv", max_length=self.max_length)
        self.X_val, self.Y_val = self.read_data(
            f"{self.file_path}/val.csv", max_length=self.max_length)
        self.X_test, self.Y_test = self.read_data(
            f"{self.file_path}/test.csv", max_length=self.max_length)

        print(
            f"train size: {len(self.X_train)}, validation size: {len(self.X_val)}, test size: {len(self.X_test)}")
    ...
    def read_data(self, path, max_length):
        ...
        sentences = sentences.apply(
            lambda x: clean_tokenize_truncate(x, max_length))
    ...
    def get_loaders(self, batch_size, num_workers, shuffle_train=False,
        ...
        database_dataset = My_Dataset(
            self.X_train, self.Y_train, self.max_length)
