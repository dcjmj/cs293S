...
class MICPQ(Base_Model):
    ...
    def define_parameters(self):
        ...
        self.bert_model = AutoModel.from_pretrained('distilbert-base-uncased', config=bert_config)
    ...
    def forward(self, inputs):
        ...
        #evaluate the similarity of codebooks (eigen vector?)
        #method1: treat it as the similarity of resort vector
        c = torch.split(self.pq_head.C, self.pq_head.L_word,
                        dim=1)  # tuple; ele of the tuple has the shape like (N_words, L_word)。 N_books elements.

        #list size is n_book, prob0 is (batch_size, N_words)
        cos1 = torch.nn.CosineSimilarity(dim=1)
        sim = 0
        for i in range(len(prob0_list) - 1):
            for j in range(0, i, 1):
                prob_i = prob0_list[i]
                prob_j = prob0_list[j]
                #prob_i_resort = prob_i[:, prob_i[0].argsort()]
                #prob_j_resort = prob_j[:, prob_j[0].argsort()]
                sim_ij = cos1(prob_i, prob_j)
                sim = sim + torch.sum(sim_ij)
                #print("sim_ij is ", sim_ij)
                #print("sim is ", sim)
        loss = self.hparams.code_weight * contra_loss + self.hparams.prob_weight * prob_loss + sim * 0.005

        #method2: direct treat the codebooks as matric
        # cos = torch.nn.CosineSimilarity(dim=0)
        # code_sim_loss = 0
        # for i in range(self.pq_head.N_books - 1):
        #     for j in range(0, i, 1):
        #         if(i!=j):
        #             c_i = c[i]
        #             c_j = c[j]
        #             diff = c_i.unsqueeze(0) - c_j.unsqueeze(0)
        #             code_sim_loss = code_sim_loss - torch.sum(torch.sum(diff * diff, - 1))
        #
        # loss = self.hparams.code_weight * contra_loss + self.hparams.prob_weight * prob_loss + code_sim_loss * 0.001

        #print("contra loss is ", contra_loss, "prob_loss is ", prob_loss, "codebook loss is ", sim * 0.01, "all loss is ", loss)
